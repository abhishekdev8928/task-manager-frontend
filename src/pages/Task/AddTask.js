...
const AddTask = () => {
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.append("assignto", JSON.stringify(formData.assignto)); // stringified

      if (selectedFile) {
        formDataToSend.append("file", selectedFile);
      }
  const adminid = localStorage.getItem("adminid");
    formDataToSend.append("createdBy", adminid); // ✅ include creator   
       const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/task/addtask`, {
        method: "POST",
        body: formDataToSend,
      });
      ...
      if (!response.ok) {
        toast.error(result.msg || "Failed to add task.");
        return;
      }
  ...
  return (
    <div className="page-content">
      <Container fluid>
        <Breadcrumbs title="ADD TASK" breadcrumbItems={breadcrumbItems} />
        <Row>
          <Col xl="12">
            <Card>
              <CardBody>
                <form onSubmit={handleAddSubmit}>
                  <Row>
                    ...
                    <Col md="6">
                      ...
                      {errors.frequency && <span className="text-danger">{errors.frequency}</span>}
                    </Col>
                    ...
                    <Col md="6">
                      ...
                      {errors.task_status && <span className="text-danger">{errors.task_status}</span>}
                    </Col>
                    ...
                    <Col md="6">
                      ...
                      <Select
                        ...
                        placeholder="Choose..."
                      />
                      {errors.assignto && <span className="text-danger">{errors.assignto}</span>}
                    </Col>
                    ...
                    <Col md="6">
                      ...
                      <Select
                        ...
                        name="client"
                        value={clientOptions.find(opt => opt.value === formData.client) || null}
                        onChange={(selected) =>
                          setFormData((prev) => ({
                            ...prev,
